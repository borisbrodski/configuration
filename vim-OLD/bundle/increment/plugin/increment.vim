...
function! IncrementColumn(pad, ...)
  ...
  let presNumString = strpart(getline('.'), c1-1, clen+1)
  " echo "STR: '" . presNumString . "'"
  if match(presNumString, '^\s*-\?\d\+$') != -1
    exe "let presNum = ".presNumString
  else
    echo "ERROR!"
    return
  endif
    ...
    if match(strpart(getline('.'), cnow-1, cnow+clen-2), '^\s*-\?\d\+$') != -1
